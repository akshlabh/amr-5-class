...
def main():
    ...
    if model_type in ('mcldnn_attention_amp',
    ...
    def prepare_inputs(X):
        ...
        if model_type == 'mcldnn_diffattention_amp_focus':
            ...
            return [
                ...
                X_amp_seq,                           # (N, 128, 4)    focused amp seq
                X_amp_global,                        # (N, 2)         PAR/p95 stats
            ]
        ...
        if model_type == 'mcldnn_attention_amp_static':
            from src.features.signal_features import extract_amplitude_static_peak_features
            X_amp_seq, X_amp_global = extract_amplitude_static_peak_features(
                X,
                qam16_boundary=qam16_amp_boundary,
            )
            return [
                ...
                np.expand_dims(X[:, 0, :], axis=2),  # (N, 128, 1)    — I channel
                np.expand_dims(X[:, 1, :], axis=2),  # (N, 128, 1)    — Q channel
                X_amp_seq,                           # (N, 128, 6)    static peak sequence
                X_amp_global,                        # (N, 7)         PAR/peak stats
            ]
        if model_type == 'mcldnn_attention_phys':
            ...
            return [
                ...
                np.expand_dims(X[:, 0, :], axis=2),  # (N, 128, 1)    — I channel
                np.expand_dims(X[:, 1, :], axis=2),  # (N, 128, 1)    — Q channel
                X_amp,                               # (N, 128, 1)    — amplitude
                X_phase,                             # (N, 128, 2)    — phase cos/sin
            ]
        if branch == 'full':
            return [
                np.expand_dims(X, axis=3),           # (N, 2, 128, 1) — IQ 2D
    ...
    if model_type == 'mcldnn_attention':
        ...
        model = build_mcldnn_diffattention_amp_focus(classes=n_classes,
                                                     dropout_rate=dropout_rate,
                                                     learning_rate=initial_lr)
        if resume_weights:
            model.load_weights(resume_weights)
        ...
        if resume_weights:
            model.load_weights(resume_weights)
    elif model_type == 'mcldnn_attention_amp_static':
        from src.models.mcldnn_attention_amp_static import build_mcldnn_attention_amp_static
        model = build_mcldnn_attention_amp_static(classes=n_classes,
                                                  dropout_rate=dropout_rate,
                                                  learning_rate=initial_lr)
        if resume_weights:
            model.load_weights(resume_weights)
