"""
mcldnn_attention_amp.py — MCLDNN-Attention with amplitude-only gated features
============================================================================
...
def build_mcldnn_attention_amp_extractor(classes: int = 5,
                                         dropout_rate: float = 0.6,
    ...
    extractor = Model(inputs=inputs,
                      ...
                      name='MCLDNN_Attention_Amp_Extractor')

    if weights_path is not None:
        if not os.path.exists(weights_path):
            raise FileNotFoundError(f"Weights not found: {weights_path}")
        extractor.load_weights(weights_path)
        print(f"[amp extractor] Loaded weights from {weights_path}")
...
if __name__ == '__main__':
    ...
    model = build_mcldnn_attention_amp(classes=5)
    model.summary()
