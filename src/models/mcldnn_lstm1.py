"""
mcldnn_lstm1.py — MCLDNN variant with only 1 LSTM layer (remove lstm_2)
========================================================================
Identical architecture to mcldnn.py EXCEPT the second LSTM layer
(return_sequences=False) is removed.  lstm_1 is changed to
return_sequences=False to produce the final sequence summary directly.

Parameter reduction vs baseline:
  Removes LSTM-2: 4×(128×128 + 128² + 128) = −131,584 params

Expected total: ~273K  (baseline ~404K, attention ~205K)
"""
...
def MCLDNN_LSTM1(weights=None,
                 ...
                 **kwargs) -> Model:
    """
    MCLDNN with a single LSTM layer (lstm_2 removed).

    The single LSTM still uses hidden_size=128 so the representational
    capacity of the hidden state is preserved.  Only the second-level
    temporal refinement is removed.
    ...
    # ── Single LSTM layer (lstm_2 removed) ───────────────────────────────────
    # return_sequences=False → LSTM produces final hidden state directly
    # hidden_size kept at 128 to preserve representational capacity
    x = LSTM(units=128, return_sequences=False, name='lstm_1',
             kernel_regularizer=_l2_lstm_reg,
             recurrent_regularizer=_l2_lstm_reg)(x)
    # x: (batch, 128)
    # lstm_2 removed — saves 131,584 parameters
    ...
    model = Model(inputs=[input1, input2, input3],
                  outputs=x, name='MCLDNN_LSTM1')

    if weights is not None:
        model.load_weights(weights)
        print(f"[MCLDNN_LSTM1] Loaded weights from {weights}")
