"""
mcldnn_attention_amp_static.py — MCLDNN-Attention with static amplitude/PAR features
============================================================================
...
def _build_graph(classes: int, dropout_rate: float, attn_dropout: float = 0.1):
    ...
    input4 : (batch, 128, 6)     static amplitude + QAM16-boundary peak channels
    ...
    input4 = Input(shape=(128, 6), name='input4_amplitude_static_peak_sequence')
...
def build_mcldnn_attention_amp_static(classes: int = 5,
                                      dropout_rate: float = 0.6,
                                      learning_rate: float = 1e-3) -> Model:
    """Build and compile the standard-loss static amplitude/PAR attention model."""
    inputs, softmax_out, _ = _build_graph(classes, dropout_rate)
    model = Model(inputs=inputs, outputs=softmax_out,
                  name='MCLDNN_Attention_Amp_Static')
...
def build_mcldnn_attention_amp_static_extractor(classes: int = 5,
                                                dropout_rate: float = 0.6,
    ...
    extractor = Model(inputs=inputs,
                      ...
                      name='MCLDNN_Attention_Amp_Static_Extractor')

    if weights_path is not None:
        if not os.path.exists(weights_path):
            raise FileNotFoundError(f"Weights not found: {weights_path}")
        extractor.load_weights(weights_path)
        print(f"[amp static extractor] Loaded weights from {weights_path}")
...
if __name__ == '__main__':
    ...
    model = build_mcldnn_attention_amp_static(classes=5)
    model.summary()
    ...
    dummy4 = np.zeros((4, 128, 6), dtype='float32')
    ...
    extractor = build_mcldnn_attention_amp_static_extractor(classes=5)
